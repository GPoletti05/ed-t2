...
typedef struct stForm{
    ...
    int stroke;
    double x, y, r, w, h;
    char strokeColor[24], fillColor[24], sw[12], txt[100];
}*FormImp;
...
Form createText(char id[], double x, double y, char strokeColor[], char fillColor[], char txt[], char sw[]){
    FormImp text = (FormImp)malloc(sizeof(struct stForm));

    strcpy(text->id, id);
    text->x = x;
    text->y = y;
    strcpy(text->strokeColor, strokeColor);
    strcpy(text->fillColor, fillColor);
    strcpy(text->txt, txt);
    strcpy(text->sw, sw);
...
void setFormCorbCorp(Form c, char corb[], char corp[]){
    FormImp form = (FormImp) c;

    strcpy(form->strokeColor, corb);
    strcpy(form->fillColor, corp);
}
