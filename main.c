...
int main(int argc, char *argv[]){
    ...
    FILE *arqGeo = NULL, *arqQuery = NULL, *arqSvg = NULL, *arqSvgQ = NULL, *arqText = NULL;//Arquivos

    char command[8];    //Armazena o comando lido do arquivo .qry
    char sufixo[32], cor[32], corb[32], corp[32], id1[32], id2[32]; //Armazena os parâmetros do arquivo .qry (string)
    double x, y, r, w, h;   //Armazena os parâmetros do arquivo .qry (coordenadas)
    ...
    char cw[12] = {"1"}, rw[12] = {"1"}, tw[12] = {"1"};   //Espessura de borda de círculos e retângulos
    bool hash;
    ...
    if(nameQuery != NULL){
        while(1){
            ...
            if(feof(arqQuery))
            ...
            else if(!strcmp(command, "pnt")){
                scanPnt(arqQuery, id1, corb, corp);
                treatPnt(arqText, figures, corb, corp, id1);
            }
            else if(!strcmp(command, "pnt*")){
                scanPnts(arqQuery, id1, id2, corb, corp);
                treatPnts(arqText, figures, corb, corp, id1, id2);
            }
            else if(!strcmp(command, "delf")){
                fscanf(arqQuery, "%s", id1);
                treatDelf(arqText, figures, id1);
            }
            else if(!strcmp(command, "delf*")){
                fscanf(arqQuery, "%s", id1);
                fscanf(arqQuery, "%s", id2);
                treatDelfs(arqText,figures, id1, id2);
            }
            else if(!strcmp(command, "dq")){
                scanDQ(arqQuery, &hash, id1, &r);
                ...
                Form circle = createCircle("0", x, y, r, "black", "none", "5");
                treatDQ(arqText, arqSvgQ, blocks, id1, circle, hash);
                fprintf(arqText, "\n");
                free(circle);
            }
                ...
                if(!strcmp(type1, "h")){
                    ...
                    x = getHydrantX(figure1);
                    y = getHydrantY(figure1);
                    printLine(arqSvgQ, x, y,  x, 10, "red");
                    printText(arqSvgQ, x, 0, id1, "red");
                    ...
                    x = getTrafficLightX(figure1);
                    y = getTrafficLightY(figure1);
                    printLine(arqSvgQ, x+10/2, y+10/2,  x+10/2, 0, "red");
                    printText(arqSvgQ, x+10/2, 10, id1, "red");
                    ...
                    x = getRadioTowerX(figure1);
                    y = getRadioTowerY(figure1);
                    printLine(arqSvgQ, x, y,  x, 0, "red");
                    printText(arqSvgQ, x, 10, id1, "red");
                }
                    ...
                    printText(arqSvgQ, x+w/2, 10, id1, "red");
                }
    ...
    /* estes comandos abaixo sao verficados e implementados depois dos demais pois evita que as novas formas adicionadas fiquem "escondidas" atras das formas originais do arquivo.*/
    if(nameQuery != NULL){
        rewind(arqQuery);
        while(1){
            fscanf(arqQuery, "%s", command);
            ...
            if(feof(arqQuery))
                ...
                scanCAR(arqQuery, &x, &y, &w, &h);
                treatCAR(arqText, arqSvgQ, x, y, w, h, blocks, hydrants,tLights,rTowers);
            }
            ...
            if(!strcmp(command, "o?")){
                ...
                Form circle = createCircle("0", x, y, r, "magenta", "none", "5");
                printDQ(arqText, arqSvgQ, circle, hash);
                fprintf(arqText, "\n");
                free(circle);
            }
        }
